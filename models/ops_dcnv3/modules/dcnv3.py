...
def build_act_layer_emb(act_layer):
    if act_layer == 'ReLU':
        return GeneralReLU(inplace=True)
    # elif act_layer == 'SiLU':
    #     return nn.SiLU(inplace=True)
    # elif act_layer == 'GELU':
    #     return nn.GELU()
...
class DCNv3_pytorch_emb(nn.Module):
    def __init__(
            self,
            channels=64,
            kernel_size=3,
    ...
    def forward(self, input, emb):
        ...
        x = self.input_proj(input, emb)
        x_proj = x

        x1 = input.permute(0, 3, 1, 2)
        for layer in self.dw_conv:
            x1 = layer(x1, emb)
        offset = self.offset(x1, emb)
        mask = self.mask(x1, emb).reshape(N, H, W, self.group, -1)
        mask = F.softmax(mask, -1).reshape(N, H, W, -1)
        ...
        if self.center_feature_scale:
            ...
            center_feature_scale = center_feature_scale[..., None].repeat(
                1, 1, 1, 1, self.channels // self.group).flatten(-2)
            x = x * (1 - center_feature_scale) + x_proj * center_feature_scale
        x = self.output_proj(x, emb)

        return x


class DCNv3Emb(nn.Module):
    def __init__(
            self,
            channels=64,
            kernel_size=3,
