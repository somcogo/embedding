...
A PyTorch implementation of the paper: `MaxViTEmb: Multi-Axis Vision Transformer`
    - MaxViTEmb: Multi-Axis Vision Transformer
...
class RelativeSelfAttention(nn.Module):
    ...
    def __init__(
        ...
        fan_in, _ = nn.init._calculate_fan_in_and_fan_out(self.qkv_weight)
    ...
    def forward(
        ...
        if len(emb.shape) == 1:
            emb = emb.unsqueeze(0)
        params = self.affine(emb)
        weight = params[0, :self.in_channels * self.in_channels * 3]
        weight= weight.reshape((3 * self.in_channels, self.in_channels))
        bias = params[0, -3 * self.in_channels:]

        qkv = nn.functional.linear(input, self.qkv_weight+weight, self.qkv_bias+bias)
        qkv = qkv.reshape(B_, N, 3, self.num_heads, -1).permute(2, 0, 3, 1, 4)
        q, k, v = qkv.unbind(0)
...
class MaxViTTransformerBlock(nn.Module):
    ...
    def forward(self, input: torch.Tensor, emb: torch.Tensor) -> torch.Tensor:
        ...
        output = input_partitioned + self.drop_path(self.attention(self.norm_1(input_partitioned), emb))
...
class MaxViTBlock(nn.Module):
    ...
    def forward(self, tuple) -> torch.Tensor:
        ...
        """
        input, emb = tuple
        output = self.grid_transformer(self.block_transformer(self.mb_conv(input), emb), emb)
        return (output, emb)
...
class MaxViTStage(nn.Module):
    ...
    def forward(self, input: torch.Tensor, emb: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            output (torch.Tensor): Output tensor of the shape [B, C_out, H // 2, W // 2].
        """
        (output, emb) = self.blocks((input, emb))
        return output


class MaxViTEmb(nn.Module):
    """ Implementation of the MaxViTEmb proposed in:
    ...
    def forward_features(self, input: torch.Tensor, site_id: torch.Tensor) -> torch.Tensor:
        ...
        """
        emb = self.embedding(site_id).to(dtype=torch.float)
        output = input
        for stage in self.stages:
            output = stage(output, emb)
        return output
    ...
    def forward(self, input: torch.Tensor, site_id: torch.Tensor) -> torch.Tensor:
        ...
        Returns:
            output (torch.Tensor): Classification output of the shape [B, num_classes].
        """
        output = self.forward_features(self.stem(input), site_id)
        output = self.forward_head(output)
        return output


def max_vit_tiny_224(**kwargs) -> MaxViTEmb:
    """ MaxViTEmb tiny for a resolution of 224 X 224"""
    return MaxViTEmb(
        depths=(2, 2, 5, 2),
...
def max_vit_small_224(**kwargs) -> MaxViTEmb:
    """ MaxViTEmb small for a resolution of 224 X 224"""
    return MaxViTEmb(
        depths=(2, 2, 5, 2),
...
def max_vit_base_224(**kwargs) -> MaxViTEmb:
    """ MaxViTEmb base for a resolution of 224 X 224"""
    return MaxViTEmb(
        depths=(2, 6, 14, 2),
...
def max_vit_large_224(**kwargs) -> MaxViTEmb:
    """ MaxViTEmb large for a resolution of 224 X 224"""
    return MaxViTEmb(
        depths=(2, 6, 14, 2),
