...
class EmbeddingTraining:
    def __init__(self, comm_rounds=500, batch_size=128, logdir='test', lr=1e-3,
                 ...
                 scheduler_mode='cosine', T_max=500, save_model=False, strategy='noembed',
                 finetuning=False, embed_dim=2, model_path=None,
                 embedding_lr=None, ffwrd_lr=None, seed=0,
                 site_indices=None, sites=None,
                 model_type=None, weight_decay=1e-5, cifar=True, state_dict=None, iterations=None,
                 feature_dims=None, fed_prox=0., norm_layer='in',
    ...
    def initModels(self, embed_dim, model_type, cifar, feature_dims, norm_layer, ft_emb_vec):
        models, self.num_classes = get_model(self.dataset, self.model_name, self.site_number, embed_dim, model_type=model_type, cifar=cifar, feature_dims=feature_dims, norm_layer=norm_layer, ft_emb_vec=ft_emb_vec)
    ...
    def initOptimizers(self, lr, finetuning, weight_decay=None, embedding_lr=None, ffwrd_lr=None):
        ...
        for model in self.models:
            if finetuning:
                assert self.strategy in ['finetuning', 'onlyemb', 'fedbn', 'embbnft', 'nomerge']
                all_names = get_layer_list(strategy=self.strategy, model=model)
    ...
    def initTensorboardWriters(self):
        tensorboard_dir = os.path.join('path/to/runs', self.logdir_name)
    ...
    def computeBatchLoss(self, batch_tup, model, metrics, mode, site_id):
        ...
        batch, labels = transform_image(batch, labels, mode, self.transforms[site_id] if hasattr(self, 'transforms') else None)
        ...
        if self.fed_prox > 0 and not self.finetuning:
            layers = get_layer_list(self.strategy, model=self.global_model)
    ...
    def saveModel(self, epoch_ndx, val_metrics, trn_dls, val_dls):
        model_file_path = os.path.join('path/to/models',
                                       self.logdir_name,
                                       f'{self.time_str}-{self.comment}.state')
        data_file_path = os.path.join('path/to/metrics',
                                      self.logdir_name,
                                      f'{self.time_str}-{self.comment}.state')
        ...
        layer_list = get_layer_list(strategy=self.strategy, model=self.models[0])
    ...
    def saveEmbs(self, comm_round, metrics):
        embedding_file_path = os.path.join('path/to/embeddings',
                                      self.logdir_name,
