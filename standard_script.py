...
if __name__ == '__main__':
    if cross_validate:
        ...
        for cross_val_id in range(5):
            config = config_fn(logdir, comment, degradation, model, model_type, dataset, cross_val_id=cross_val_id, ft_strategy=ft_strategy)
        ...
        config = config_fn(logdir, comment, degradation, model, model_type, dataset, cross_val_id=cross_val_id, strategy=strategy, ft_strategy=ft_strategy)
        main(**config)