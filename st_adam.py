...
class Adam(Optimizer):
    ...
    def __init__(self, params, lr=1e-3, betas=(0.9, 0.999), eps=1e-8,
                 weight_decay=0, mu=0, amsgrad=False, *, foreach: Optional[bool] = None,
        ...
        defaults = dict(lr=lr, betas=betas, eps=eps,
                        weight_decay=weight_decay, mu=mu, amsgrad=amsgrad,
        ...
        for group in self.param_groups:
            ...
            for p in group['params']:
                if p.grad is not None:
                    ...
                    if hasattr(p, "global_weight"):
                        global_weights.append(p.global_weight)
                    else:
                        global_weights.append(None)
...
def _single_tensor_adam(params: List[Tensor],
    ...
    for i, param in enumerate(params):
        ...
        global_weight = global_weights[i]
        exp_avg = exp_avgs[i]
        ...
        if capturable:
            ...
            if global_weight is not None:
                # param = (param + global_weight * fact) / (1 + fact)
                # needs to be inplace
                fact = mu/denom.abs()
                param.add_(global_weight * fact)
                param.div_(1 + fact)
                param = (param * denom.abs() + global_weight * mu) / (denom.abs() + mu)
        else:
            step = step_t.item()
